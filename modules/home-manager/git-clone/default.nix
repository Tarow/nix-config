...
in
{
  ...
  config = lib.mkIf (cfg.repos != { }) {
    ...
    home.activation.git-repos = hm.dag.entryAfter [ "writeBoundary" ]
      (concatStringsSep "\n"
        (mapAttrsToList
          (name: repo:
            (
              let dir = builtins.replaceStrings [ "//" ] [ "/" ] "${repo.location}/${name}"; in
              if repo.enable then ''run [ -d "${dir}" ] || git clone "${repo.uri}" "${dir}" '' else ''run rm -rf "${dir}" ''
            )
          )
          cfg.repos));
  };
